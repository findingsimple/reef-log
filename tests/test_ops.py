...
def test_add_test_session_rejects_unknown_tank(conn: sqlite3.Connection):
    with pytest.raises(ValueError, match="unknown tank"):
        ops.add_test_session(
            conn, [{"parameter": "alkalinity", "value": 8.2}], tank="dispaly"
        )
...
def test_add_test_session_rejects_shared_tank(conn: sqlite3.Connection):
    ...
    with pytest.raises(ValueError, match="unknown tank"):
        ops.add_test_session(
            conn, [{"parameter": "alkalinity", "value": 8.2}], tank="both"
        )
...
def test_get_recent_real_tank_includes_shared_maintenance(conn: sqlite3.Connection):
    ops.add_maintenance(
        conn, "filter_change", tank="both", performed_at=_dt(days_ago=1)
    )
    ops.add_maintenance(
        conn, "water_change", tank="display", performed_at=_dt(days_ago=2)
    )
    ops.add_maintenance(
        conn, "water_change", tank="frag", performed_at=_dt(days_ago=3)
    )
